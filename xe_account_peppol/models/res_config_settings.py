...
class ResConfigSettings(models.TransientModel):
    ...
    def action_validate_peppol(self, company_id):
        ...
        '''
        company_id = self.company_id or company_id or self.env.company
        if not company_id.account_peppol_edi_api_key:
        ...
        try:
            response = company_id._make_request(
                ...
                payload={'api_key': company_id.account_peppol_edi_api_key}, headers=HEADERS, method="POST"
            )
    ...
    def action_regenerate_tokens(self, company_id=False):
        ...
        try:
            ...
            if response.status_code == 400:
                return self.action_validate_peppol(company_id)
            if not (200 <= response.status_code <= 299):
                if json_response.get('message'):
                    raise AccessError(json_response.get('message'))
        except Exception as e:
